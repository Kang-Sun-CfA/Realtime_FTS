...
input_LBL.which_CIA = {'gfit','sao'};% keep lower case
...
input_LBL.taumol = 'CO2';
input_LBL.profile = ones(size(input_LBL.P_layer))*390e-6;% 390 ppm
output_LBL_CO2 = F_line_by_line(input_LBL);
...
input_nlinfit.s1 = nan(4,length(common_grid));
...
input_nlinfit.s1(4,:) = squeeze(sum(output_LBL_CO2.dtau_layer(1,:,:),3));% CO2
...
ylabel('Spectra')
subplot(2,1,2)
h = plot([wStart wEnd],[0 0],'--k',w2,R_sao,w2,R_gfit);
ylim([-0.02 0.02]);
legend(h(2:3),'SAO','GFIT')
xlabel('Wavenumber')
ylabel('Residuals')
%%
addpath('~/matlab functions/export_fig/')
export_fig 'fitting_residuals_iso_106.pdf'
%%
close all
...
legend('SAO','GFIT')
%%
export_fig 'CIA_comparison_iso_106.pdf'
%%
