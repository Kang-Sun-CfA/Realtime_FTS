...
inp_tau.which_CIA = {'sao','gfit','mate','hitran'};
...
dgrd_fwhm = 2.5*inp_tau.common_grid_resolution;

%% load profile
fid = fopen(inp_tau.profile_fn);
...
C_low(1) = inp_tau.lowest_possible_Psurf;
for i = 2:size(C_0,2)
    C_low(i) = interp1(C_0(:,1),C_0(:,i),inp_tau.lowest_possible_Psurf);
end

C = cat(1,C_0(C_0(:,1)-inp_tau.lowest_possible_Psurf < 0,:),C_low);

nsublayer = inp_tau.nsublayer;
nlevel = size(C,1);
nlayer = nlevel-1;
%% make some plots
if ispc
