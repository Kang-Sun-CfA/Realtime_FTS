...
for iwin = 1:length(window_list)
    ...
    for imol = 1:length(mol_for_fit)
        ...
        % read absco data
        if molN == 1 && if_broadening
            varname_b = cat(1,varname(:),'Broadener_01_VMR');
        else
            varname_b = varname;
        end
        absco = F_read_hdf(absco_fn,varname_b);
        inp_interp = [];
        ...
        % add H2O broadening for H2O itself only, for now. waiting for
        % Yan's CO2/CH4 water broadening
        if molN == 1 && if_broadening
        inp_interp.bgrid = absco.Broadener_01_VMR.data;
        end
